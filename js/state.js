...
let UNIT_GAME     = null;        // 학생: 단원에서 연 실습 게임 id (null 이면 게임 화면 아님)
...
let AIC_VIEW         = 'entry'; // 학생: 'entry'|'brief'|'chat'|'reflect' / 선생님: 'manage'|'student'
let AIC_BRIEF        = null;    // 학생 설계 브리프 {problem, why, connect, io, ctrl}
let AIC_BRIEF_EDITING= false;   // 채팅 도중 설계 수정 모드 (대화·코드 유지)
let AIC_RUNS_LOG     = [];      // 실행 기록 [{ts, ok, err}] — 시행착오 타임라인 (세특 근거)
let AIC_REFLECT      = null;    // 마무리 성찰 {name, intro, hard, learned, next, ts}
let AIC_DONE         = false;   // 완성 선언 여부
let AIC_CHAL_USED    = false;   // 확장 도전 카드 사용 여부
...
// 테스트 단계: 학생이 캔버스에 직접 그림 → 모델이 맞춤
let ML_SUP_DRAW_STROKES = [];       // [{ color, size, points:[[x,y],...] }] - 그린 획들
let ML_SUP_DRAW_COLOR   = '#e23b3b';// 현재 펜 색
let ML_SUP_DRAW_SIZE    = 16;       // 펜 굵기
let ML_SUP_DRAW_PRED    = null;     // 마지막 예측 { classId, label, probs }
// 비지도학습 상태
...
// 선생님: 응시 관리·채점·편집
let MLA_TC_VIEW    = 'list'; // 'list'|'student'|'edit'
...
// AI 학습지 (인공지능 단원) — 정의는 aiactivity-data.js
