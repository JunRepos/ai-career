...
function scheduleNBSave(){
  ...
  // 학생 진도 보는 중에는 저장 안 함 (다른 학생 데이터 보호)
  if(NB_VIEWING_STUDENT) return;

  // ── 선생님: 원본 노트북 자동 저장 ──
  //   학생이 다음에 열 때 셀 ID 매칭으로 머지되어 변경사항 반영
  //   학생 작성 내용은 보존 (openNotebook의 _mergeNotebookCells 가 처리)
  if(IS_TC && TC_CLS){
    ...
    _nbSaveTimer = setTimeout(async () => {
      ...
      try {
        const sanitized = (NB_CELLS || []).map(c => ({
          id: c.id || '',
          type: c.type || 'code',
          source: c.source || ''
        }));
        await db.ref(`notebooks/${TC_CLS.id}/${SEL_NOTEBOOK.id}`).update({cells: sanitized});
        // 메모리 캐시도 동기화
        SEL_NOTEBOOK.cells = sanitized;
        const cached = NOTEBOOKS.find(n => n.id === SEL_NOTEBOOK.id);
        if(cached) cached.cells = sanitized;
        setNBSaveStatus('saved');
      } catch(e){
        console.error('선생님 노트북 저장 실패:', e);
...
// ── 노트북 열기 ──
// 학생 진도와 원본 노트북을 셀 ID 기반으로 머지
//   - 학생이 편집한 셀: 학생 source 그대로 보존
//   - 선생님이 새로 추가한 셀: 원본 위치에 끼워 넣음
//   - 선생님이 삭제한 셀: 학생이 작업한 내용 손실 방지 위해 그대로 유지 (끝에 모임)
//   - 학생이 직접 추가한 셀: 그대로 유지
function _mergeNotebookCells(origCells, savedCells){
  const orig = origCells || [];
  const saved = savedCells || [];
  // 학생 진도를 id 기준 맵으로
  const savedById = new Map();
  saved.forEach(c => { if(c.id) savedById.set(c.id, c); });

  const result = [];
  const usedIds = new Set();

  // 1) 원본 순서대로 — 학생이 편집한 셀이면 학생 source 사용
  for(const oc of orig){
    if(oc.id && savedById.has(oc.id)){
      const sc = savedById.get(oc.id);
      result.push({
        id: oc.id,
        type: sc.type || oc.type || oc.cell_type || 'code',
        source: sc.source != null ? sc.source : (oc.source || '')
      });
      usedIds.add(oc.id);
    } else {
      // 선생님이 새로 추가한 셀 — 학생 진도에 없음 → 원본 그대로
      result.push({
        id: oc.id,
        type: oc.type || oc.cell_type || 'code',
        source: oc.source || ''
      });
    }
  }

  // 2) 학생만 가지고 있는 셀 (학생이 추가했거나 선생님이 삭제한 셀) → 끝에 보존
  for(const sc of saved){
    if(!sc.id || !usedIds.has(sc.id)){
      result.push({
        id: sc.id || `cell-orphan-${Math.random().toString(36).slice(2,8)}`,
        type: sc.type || 'code',
        source: sc.source || ''
      });
    }
...
async function openNotebook(nbId){
  ...
  // 머지: 학생 진도가 있으면 원본의 새 셀을 자동 반영하되 학생 편집 내용 보존
  // (학생 진도가 없으면 원본 그대로)
  let sourceCells;
  let teacherUpdated = false;
  if(savedCells){
    sourceCells = _mergeNotebookCells(nb.cells || [], savedCells);
    // 선생님이 새 셀을 추가했는지 감지 (학생 진도에 없던 원본 셀 ID가 있으면 신규)
    const savedIds = new Set(savedCells.map(c => c.id).filter(Boolean));
    teacherUpdated = (nb.cells || []).some(c => c.id && !savedIds.has(c.id));
  } else {
    sourceCells = nb.cells || [];
  }
  ...
  // 선생님이 노트북 업데이트한 경우 학생에게 알림
  if(teacherUpdated){
    setTimeout(() => toast('🆕 선생님이 노트북을 업데이트했어요. 새 셀이 추가됐습니다 (기존 작성 내용은 보존됨).', 'info'), 300);
  }
}
