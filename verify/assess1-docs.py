...
⚠ 평가요소 이름 4개 · 배점 5·5·5·5 · 기본점수 8점은 계획서 6-가 그대로입니다.
   ...
   사이트 문항마다 그 문항의 결격 사유를 「📌 채점 기준」 으로 굵게 · 색으로 보여 줍니다 (ITEM_CRIT).
   작성 안내는 2026-09-14 오후 선생님 「너무 구구절절」 로 걷어냈습니다 (조건은 종이 평가지에만).
...
# 사이트 문항마다 보여 줄 채점 기준 — (평가 요소, 결격 사유 번호들). 1 · 2번은 요소 ① 을 나눠 가집니다.
ITEM_CRIT = {'1': ('①', [0, 1]), '2': ('①', [2, 3]), '3': ('②', [0, 1, 2]), '4': ('③', [0, 1]), '5': ('④', [0, 1])}
    ...
    ('①', '문제 상황을 탐색하고 필요한 인공지능 구상하기', '1 · 2번',
     ['기사 · 글 · 영상에서 찾았는데 **출처(링크)**를 밝히지 않음',
      ...
      '**지금의 해결 방식과 그 한계**를 들지 않고 인공지능이 필요한 까닭을 서술하거나, **까닭이 없음**'], [
        (5, '인공지능으로 해결할 수 있는 문제 상황을 선정하여 출처, 실생활이나 진로와의 관계, 어려움을 겪는 대상과 어려움, '
...
def defects_html(df):
    return '<br>'.join('%s %s' % (MARK[i], re.sub(r'\*\*(.+?)\*\*', r'<b>\1</b>', d)) for i, d in enumerate(df))
...
def sheet():
    ...
    b1 = block(0, mid='<div class="res"><span>선정한 문제 상황</span><span class="rl"></span></div>'
                      '<div class="res"><span>출처 (링크 또는 기사 제목 · 매체 · 날짜)</span><span class="rl"></span></div>')
    b2 = block(1)
...
<div class="ex">
<p><b>1번. 문제 상황 선정</b> &nbsp;<u>선정한 문제 상황</u> 흉부 엑스레이 판독이 늦어지고 작은 결절을 놓친다.
<u>링크</u> 기사 「건강검진 엑스레이 판독 지연, 결절 놓쳐」(○○일보, 2026. 8.)의 주소. <u>선정한 동기</u> 나는 영상의학과에
...
<tr><td class="center">동기</td><td class="center">1</td><td>선정한 동기와 출처</td></tr>
...
#   2026-09-14 칸을 나눔 — 1번 셋(문제 상황 한 줄 · 링크 · 선정 동기 — 링크 칸만 붙여넣기 허용), 2번 둘, 4번 둘. 옛 칸 id 는 그대로 두고 새 칸만 더함.
APP_FIELDS = {
    '1': [dict(id='q1title', label='선정한 문제 상황 (한 줄)', type='text'),
          dict(id='q1link', label='링크 — 기사 · 글 · 영상에서 찾았다면 그 주소를 붙여 넣기', type='text', paste=True, ph='https://'),
          dict(id='q1', label='선정한 동기 (실생활 연결 또는 진로 연결)', type='area', rows=6)],
...
def app_data():
    SC_DEF = {no: df for no, name, where, df, lv in SCALE}
    items = []
    for no, title, pt, elem, ask, conds, lines in ITEMS:
        ...
        items.append(dict(no=no, title=title, pt=pt, showPt=(no != '1'), ask=_plain(ask),
                          ...
                          crit=dict(area=ITEM_CRIT[no][0], shared=(no in ('1', '2')),
                                    items=[[MARK[k], SC_DEF[ITEM_CRIT[no][0]][k]] for k in ITEM_CRIT[no][1]])))
