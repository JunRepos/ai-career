...
### `side` — 그림을 보면서 설명하기
`tree` 와 `diagram` 에 `side:[{label,text,accent,big}]` 를 주면
**왼쪽에 그림을 그대로 두고** 오른쪽에 설명 카드를 세웁니다.
용어를 설명할 때는 반드시 이 방식을 쓰세요 — 학생이 그림과 대조하며 들어야 합니다.

**`big:true`** — 오픈/닫힌 리스트처럼 **값 자체를 읽어야 하는** 칸입니다.
30pt 에서 시작해 필요한 만큼만 줄입니다(기본은 21pt). 글자색도 진하게 나옵니다.
이런 칸은 **카드를 3개 이하로** 두세요. 카드가 많으면 칸이 낮아져 글자가 다시 줄어듭니다.
